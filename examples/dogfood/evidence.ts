...
export const jsonl = (path: string): any[] => {
  const text = readFileSync(path, 'utf8');
  assert(text.endsWith('\n'), 'unterminated JSONL');
  return text.trimEnd().split('\n').map(line => JSON.parse(line));
};
/** Compare stock persistent records with authoritative stream messages, not cumulative deltas. */
export function checkRecording(session: any[], events: any[]) {
  assert.equal(session[0]?.type, 'session', 'session header');
  assert.equal(events[0]?.id, session[0].id, 'stream/session identity');
  assert(session.some(e => e.type === 'thinking_level_change' && e.thinkingLevel === 'max'), 'effective effort');
  assert(session.some(e => e.type === 'model_change' && e.provider === 'ai-gw-baseten' && e.modelId === 'baseten/deepseek-ai/DeepSeek-V4-Flash-0731'), 'frozen model');
  const messages = session.filter(e => e.type === 'message').map(e => e.message);
  assert(messages.some(m => m.role === 'user'), 'prompt population');
  ...
  for (const call of calls) {
    ...
    assert(!JSON.stringify(result[0]).match(/Full output saved|Output truncated|Showing lines|"truncated":true/), 'truncated output requires recovery');
  }
