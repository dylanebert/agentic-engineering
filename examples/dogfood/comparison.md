...
| Marked Reference | Applied Channel | Concrete Comparison | Limit |
|---|---|---|---|
| Karpathy, *A Recipe for Training Neural Networks*, pinned prose capture | Failure/check motivation; imperative sequence | The reading opens with passing behavior versus a broken context protocol. The replay's real simulation error precedes the independent browser check. It does not invent a note bug to motivate testing. | Training guidance and hardware practices are not transferred. This is a testing mistake, not an application defect. |
...
The current `src/App.svelte` has `loop` and `verification` sections. `src/lib/figures.ts` places the existing StageLoop after the second loop paragraph. These are reference surfaces only; no article, manifest, manuscript, page chrome or figure was changed.
...
| Replay spec entry `1744793a` | Read a short spec before implementing | Model-authored storage choice and direct-file wording | Human recipe uses HTTP; file-URL behavior is unobserved |
| Replay first-stage browser observations | Check empty, multiline, Clear | Full app hashes, actual values, screenshot and close records | Serving and browser work happen outside the model |
| Replay `629b8110` | A shell fallback mislabeled a warning | `tidy` output and successful overall command status | Tool exit totals do not capture every warning |
| Replay `cdbf5b3f` → `d098c486` → `2f5980be` | The model fixed its simulated click target | Error, exact repair statement and printed results | Test code can be wrong even when app code is correct |
| Replay `d2d1ddb8` and final browser records | Printed values are not six assertions | Inaccurate model claim beside the real seven browser assertions | Do not substitute a completion summary for evidence |
| Canary/control/observer-red corpus | Check the observer too | Deliberate negatives, terminal-event repair, teardown rechecks, archive repair | These are preparation costs, not model application failures |
| Usage/timing summaries and price reference | Separate subject time from total effort | All attempts, canaries, unknown gateway bill and token-accounting limits | A cheap token illustration is not the cost of making this example |

The table proposes one sequence, not a blend of successful fragments from different attempts. The complete original and adjusted attempts remain disclosed beside the selected replay. The final replay was not another adjustment, and there is no fourth attempt available.

## Reproduce the Comparison

From the article member, with the separately supplied marked captures available:

```sh
bun examples/dogfood/compare.ts /absolute/kex/reference/taste/captures
```
...
The original comparison found one long pricing paragraph. During known repairs, the supplement introduction also reached 82 words and was split without dropping qualifications; the final comparison exits 0. The lexical scan now reports 34 candidates across 4,040 prose words, 8.42 per thousand, below the 12-per-thousand tripwire even before classifying non-relative uses. Code, tables and block quotations are excluded; recipe prose is included. This is a lexical check, not a semantic or educational verdict.
