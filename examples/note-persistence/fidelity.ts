...
export const requiredDisclosures = [
  ...
  'Both standalone readings were rejected; functional success is not prose acceptance.',
...
/** These are fidelity predicates, not a style score or acceptance of the rejected readings. */
...
export function checkFidelity(root: string, detail: string, mapping: ReturnType<typeof sources>) {
  ...
  assert.equal(human.educationalOutcome, 'both standalone readings rejected; review mechanism retired', 'separate educational decision');
  assert.equal(human.functionalVerbatim, 'it works', 'verbatim functional response');
  assert.equal(human.structuralAcceptance, false, 'no invented structural acceptance');
  assert.equal(human.revisedReadingCommit, '0b90c9f8a7bf3bf3e6f37c4cb5bf2aa45dd16caf', 'second rejected reading identity');
  assert.equal(human.revisedReadingQuote, "I'm not reading all that. it looks like your own internal notes. why should I care? my judgement should be on the structure of what's presented in the blog post.", 'actual mechanism rejection');
  assert(detail.includes(`“${human.revisedReadingQuote}”`), 'verbatim mechanism rejection in detail');
  assert(detail.includes(`${human.revisedReadingCommit}/examples/note-persistence/reading.md`), 'second rejected reading pin');
