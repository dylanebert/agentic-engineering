...
/opt/homebrew/bin/node -p 'new Date().toISOString()' > "$ROOT/support/records/no-model-stdin-start.txt"
/usr/bin/time -p env -i HOME="$ROOT/support/home" PATH=/Users/dylan.ebert/.bun/bin:/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin TMPDIR="$ROOT/support/tmp" PI_CODING_AGENT_DIR="$ROOT/support/config" PI_OFFLINE=1 PI_TELEMETRY=0 PLAYWRIGHT_BROWSERS_PATH="$ROOT/acquired-starter/browsers" DEBUG=pw:browser,pw:webserver DEBUG_COLORS=0 \
  ...
  --session "$ROOT/support/records/no-model-stdin.jsonl" --mode json -p < "$ROOT/support/implementation-prompt.txt" \
  > "$ROOT/support/records/no-model-stdin-events.jsonl" 2> "$ROOT/support/records/no-model-stdin-stderr.log"
printf '%s\n' "$?" > "$ROOT/support/records/no-model-stdin.exit"
/opt/homebrew/bin/node -p 'new Date().toISOString()' > "$ROOT/support/records/no-model-stdin-end.txt"
