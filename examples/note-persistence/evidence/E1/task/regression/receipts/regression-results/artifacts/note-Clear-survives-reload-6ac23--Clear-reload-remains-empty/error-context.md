...
- Name: note.spec.ts >> Clear survives reload and another Clear/reload remains empty
- Location: tests/note.spec.ts:37:1
...
Call log:
  ...
  - waiting for locator('textarea#note')
    10 × locator resolved to <textarea rows="8" id="note" cols="40"></textarea>
       - unexpected value "to be cleared"

```

```yaml
- textbox "Your note": to be cleared
```
