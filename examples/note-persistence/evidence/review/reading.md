...
A note page lets you type and Clear, but loses your words when you reload. I supplied that working page and the browser-test setup, then asked an agent to add persistence. I supplied no saving code, task spec or test assertions.
...
There was setup help: two launcher failures happened before the implementation conversation started. I diagnosed them without model calls and fed the same prompt through Pi's standard input instead. That repair and the supplied browser setup are part of the example, not work Luna did.
...
Tests preserve their assumptions too. A fresh agent review is still pending, followed by a person's real note-taking and Clear. Those judgments are separate from passing assertions. Storage denial, other browser engines and cross-tab behavior weren't tested.
