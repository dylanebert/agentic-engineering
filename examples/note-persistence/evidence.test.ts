...
test('shorter reading preserves every semantic obligation and the actual human quote', () => {
  ...
  expect(readJson(join(root, 'campaign.json')).humanDecision).toBe('functional use confirmed; both standalone readings rejected; article judgment separate');
});
...
test('production comparison CLI refuses a removed intervention and changed outcome', () => {
  ...
  try {
    ...
    const cases = [
      ...
      ['false-acceptance', original.replace("I'm not reading all that. it looks like your own internal notes. why should I care? my judgement should be on the structure of what's presented in the blog post.", 'The explanation is accepted.'), 1, 'verbatim mechanism rejection in detail'],
      ['restored', original, 0, ''],
    ] as const;
