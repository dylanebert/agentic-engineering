<script lang="ts">
  ...
  let clock = $state(0); let drawn = $state(false); let live = $state(false);
  ...
  onMount(() => {
    ...
    const tick = (time: number) => {
      if (!active || disposed || !drawn) return;
      const dt = last ? time - last : 0;
      last = time;
      if (!live) {
        reveal += dt;
        if (reveal >= 200) live = true;
      } else {
        clock = (clock + dt) % loop;
        render(dt);
      }
    ...
    const observer = new IntersectionObserver(([entry]) => {
      ...
      void initialization.then(() => {
        ...
        if (drawn && !reduced) raf = requestAnimationFrame(tick);
      });
    }, { threshold: 0.2 });
...
<div class="hero" bind:this={root} data-hero-id="spectrum-hero" data-hero-state="agentic" style:--phase={phase} style:--veil={veil} aria-hidden="true">
  <div class="canvas-wrap"><img src={poster} alt="" width="560" height="560" data-hero-poster class:hidden={live} /><canvas bind:this={canvas} data-hero-canvas class:drawn></canvas></div>
  <svg class="spectrum" viewBox="0 0 548 64">
    ...
    <defs><filter id="hero-ink" color-interpolation-filters="sRGB">
      <feColorMatrix type="matrix" values="0 0 0 0 .122  0 0 0 0 .435  0 0 0 0 .361  .2126 .7152 .0722 0 0" />
    </filter></defs>
  </svg>
  <div class="captions">
